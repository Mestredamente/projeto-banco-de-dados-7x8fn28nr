...
export default function Patients() {
  ...
  const loadData = async () => {
    try {
      const records = await pb.collection('patients').getFullList({ sort: 'name' })
      setPatients(records)
    } catch (e) {
      console.error(e)
    }
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 mt-1">Gerencie a base de pacientes da clínica.</p>
        </div>
              ...
              {filteredPatients.length === 0 ? (
                ...
                filteredPatients.map((p) => (
                  <TableRow key={p.id}>
                    ...
                    <TableCell>
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-semibold ${p.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                      >
